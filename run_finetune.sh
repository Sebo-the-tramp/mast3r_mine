CUDA_VISIBLE_DEVICES=0,1,2,3 torchrun --nproc_per_node 4 --master_addr=127.0.0.1 --master_port=29502 train.py \
    --train_dataset="100_000 @ ARKitScenes(split='train', aug_crop=256, resolution=[(512, 384), (512, 336), (512, 288), (512, 256), (512, 160)], transform=ColorJitter, ROOT='/l/users/sebastian.cavada/datasets/arkit/arkitscenes_test', seed=777, n_corres=8192, nneg=0.5)" \
    --test_dataset "10_000 @ ARKitScenes(split='test', aug_crop=256, resolution=(512, 384), transform=ColorJitter, ROOT='/l/users/sebastian.cavada/datasets/arkit/arkitscenes_test', seed=777, n_corres=1024)" \
    ...
    --output_dir="checkpoints/dust3r_512dpt_finetuned_100k_mod"
