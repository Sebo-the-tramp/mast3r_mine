...
CUDA_VISIBLE_DEVICES=4,5,6,7 torchrun --nproc_per_node 4 --master_addr=127.0.0.1 --master_port=29502 train.py \
    ...
    --train_criterion "ConfLoss(Regr3D(L21, norm_mode='?avg_dis'), alpha=0.2) + 0.075*ConfMatchingLoss(MatchingLoss(InfoNCE(mode='proper', temperature=0.05), negatives_padding=0, blocksize=8192), alpha=10.0, confmode='mean') + 0.1 * ReprojectionLoss(L21, norm_mode='?avg_dis')" \
    ...
    --output_dir="/data3/sebastian.cavada/experiments/small/dust3r_512dpt_finetuned_100k_mod_ext+int_reproj_BEFORE"
